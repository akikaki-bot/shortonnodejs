...
PC.prototype.cpu = function(){
  return new Promise(function(resolve, reject){
    ...
    execP('wmic cpu get deviceid').then(function(stdout){

      var cpu = {}, deviceIds = [], container = [], promiseList = [];

      transforArr(stdout, deviceIds);
      var device = deviceIds[0];

      promiseList.push(execP('wmic cpu where "deviceid=\''+device+'\'" get cpustatus'));
      promiseList.push(execP('wmic cpu where "deviceid=\''+device+'\'" get currentclockspeed'));
      promiseList.push(execP('wmic cpu where "deviceid=\''+device+'\'" get currentvoltage'));
      promiseList.push(execP('wmic cpu where "deviceid=\''+device+'\'" get loadpercentage'));
      promiseList.push(execP('wmic cpu where "deviceid=\''+device+'\'" get maxclockspeed'));
      promiseList.push(execP('wmic cpu where "deviceid=\''+device+'\'" get name'));
      promiseList.push(execP('wmic cpu where "deviceid=\''+device+'\'" get numberofcores'));
      promiseList.push(execP('wmic cpu where "deviceid=\''+device+'\'" get ProcessorId'));
      ...
      Promise.all(promiseList).then(function(results){
        results.forEach(function(responseStr){
          ...
          cpu = {
            ...
            currentClockSpeed: container.shift(),
            currentVoltage: container.shift(),
            loadPercentage: container.shift(),
            maxClockSpeed: container.shift(),
            name: container.shift(),
            numberOfCores: container.shift(),
            processorId: container.shift()
          };
        resolve(cpu);
      }, errorHandler);
    }, errorHandler);
...
PC.prototype.publicIP = function(){
  return new Promise(function(resolve,reject){
    ...
    var rq = http.get(url, function(r) {
      r.setEncoding('utf8');
      r.on('data', function(data){resolve(data.replace(/\n/g, ''));});
...
PC.prototype.getAll = function(){
  ...
  return new Promise(function(resolve, reject){
    ...
    self.publicIP().then(function(ipData){
      ...
      Promise.all([self.drives(), self.users(), self.mac(), self.share(), self.programs(), self.cpu()]).then(function(info){
        resolve({
          memory: self.memory(),
...
PC.prototype.get = function(listArr){
  ...
  return new Promise(function(resolve, reject){
    ...
    nonPromiseItems = ['memory', 'hostname', 'uptime', 'networkInterface'],
